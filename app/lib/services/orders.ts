...
/**
 * Calculate salary based on source rules for a specific period
 */
export async function calculateSalaryForPeriod(startDate: string, endDate: string, employeeId?: string) {
  try {
    console.log(`Calculating salary for period: ${startDate} to ${endDate}...`);
    ...
    const filter = employeeId 
      ? `created >= "${startDate}" && created <= "${endDate}" && status.name = "completed" && assignedTo = "${employeeId}"`
      : `created >= "${startDate}" && created <= "${endDate}" && status.name = "completed"`;
    
    const orders = await authenticatedCall(() => 
      pb.collection('orders').getList(1, 1000, { 
        filter: filter,
        expand: 'source',
        fields: 'amount,source,productionCost,expand,id,orderNumber'
      })
    );
    
    // Calculate salary based on source commission percentage
    let totalSalary = 0;
    const breakdownBySource: Record<string, { commission: number, orders: string[] }> = {};
    const orderDetails: Array<{id: string, orderNumber: string, amount: number, commission: number, sourceName: string}> = [];
    
    for (const order of orders.items) {
      const sourceName = order.expand?.source?.name || 'Unknown';
      const amount = order.amount || 0;
      const productionCost = order.productionCost || 0;
      
      // Get commission percentage from source, default to source-specific rates if not set
      let commissionPercentage = order.expand?.source?.commissionPercentage;
      
      if (commissionPercentage === undefined || commissionPercentage === null) {
        // Use default percentages based on source name
        switch(sourceName.toLowerCase()) {
          ...
          case 'shafa':
            ...
            break;
          default: // website, instagram, facebook, viber, telegram
            commissionPercentage = 10;
            break;
        }
      }
      
      // Calculate commission after subtracting production cost
      const baseAmount = amount - productionCost;
      const commission = baseAmount > 0 ? baseAmount * (commissionPercentage / 100) : 0;
      
      totalSalary += commission;
      
      // Add to breakdown
      if (!breakdownBySource[sourceName]) {
        breakdownBySource[sourceName] = { commission: 0, orders: [] };
      }
      breakdownBySource[sourceName].commission += commission;
      breakdownBySource[sourceName].orders.push(order.id);
    ...
    return {
      ...
      ordersProcessed: orders.totalItems,
      breakdown: Object.entries(breakdownBySource).map(([source, data]) => ({
        source,
    ...
    console.error(`Error calculating salary for period ${startDate} to ${endDate}:`, error);
    return { error: 'Failed to calculate salary', details: String(error) };
  }
}
