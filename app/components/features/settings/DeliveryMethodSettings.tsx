...
export function DeliveryMethodSettings() {
  ...
  const [methods, setMethods] = useState<DeliveryOptionsResponse[]>([]);
  ...
  const fields = [
    { 
      ...
      placeholder: t('deliveryMethodNamePlaceholder') 
    }
  ];
  ...
  useEffect(() => {
    const fetchMethods = async () => {
      try {
        ...
        toast.error(t('error'), {
          description: t('deliveryMethodLoadError'),
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSave = async (data: DeliveryMethodUpdateData) => {
    try {
      ...
      toast.error(t('saveError'), {
        description: t('deliveryMethodUpdateError'),
      });
    }
  };
            ...
            >
              <CollapsibleTrigger asChild>
                <div className="flex items-center justify-between cursor-pointer hover:bg-accent/50 p-2 rounded-md">
                  <h3 className="text-base font-medium">{t('addDeliveryMethod')}</h3>
                  <Button
                    variant="ghost"
                    size="sm"
                    className="w-9 p-0"
